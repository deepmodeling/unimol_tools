...
class MolGeneration:
    def __init__(self, train_path, dict_path=None, vae_dict_path=None, unimol_weight_path=None, **kwargs):
    ...
    def train(self):
        train_dataset = VAEDataset(self.config.dataset.train_path, self.dictionary, self.vae_dict)
        valid_dataset = VAEDataset(self.config.dataset.valid_path, self.dictionary, self.vae_dict) if self.config.dataset.valid_path else None
        ...
        loss_fn = VAELoss(beta=self.config.training.beta, pad_idx=self.vae_dict.pad(), token_weights=self.token_weights)
    ...
    def generate(self, num_samples=10, checkpoint_path=None):
        ...
        model.eval()
        # Sampling logic
        # z = torch.randn(num_samples, self.config.model.latent_dim)
        # Decode z -> smiles
        pass
...
if __name__ == "__main__":
    ...
    generator = MolGeneration(train_path=args.train_path, dict_path=args.dict_path, unimol_weight_path=args.unimol_weight_path, output_dir=args.output_dir)
    generator.train()
